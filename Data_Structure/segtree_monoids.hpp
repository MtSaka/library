...
namespace monoid_lazy_segtree{
  ...
  template<class T>static constexpr S<T> mapping2(T a,S<T>b){if(a!=INF<T>)b.value=a*b.size;return b;}
  ...
  template<class T>static constexpr T composition1(T a,T b){reutrn a==INF<T>?b:a;}
